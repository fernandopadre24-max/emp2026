...
const AccountForm: React.FC<AccountFormProps> = ({ addAccount, closeModal }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || isNaN(initialBalance)) {
        alert("Por favor, preencha todos os campos com valores válidos.");
        return;
    }
    addAccount({ name, balance: initialBalance });
    closeModal();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium text-text-secondary mb-1">Saldo Inicial (R$)</label>
        <input type="number" step="0.01" value={balance} onChange={(e) => setBalance(e.target.value)} className={inputStyles} required />
      </div>
      <div className="flex justify-end space-x-2 pt-4">
        ...
        <button type="submit" className="px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-hover">Salvar Conta</button>
      </div>
    </form>
  );
};
